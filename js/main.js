!function(t) {
    ...
    function c(n) {
        ...
        if (r && 0 != r && (u += "&environment=" + r), u += "&lang=" + i, u += "&variant=" + l, c && (u += "&versionTag=" + c), !o)
            return Raven.captureException(new Error("Attempt to fetch examples without any valid collection UID available"), {
                extra: {
                    api: u,
                    id: o,
                    ownerId: ownerId
            ...
            return e ? a(e) : (n && "unauthorized" === n.status && function(e) {
                if (e) {
                    ...
                    for (var o = 0; o < n.length; o++)
                        if (n[o].key === t && n[o].variant === a)
                            return n[o] = e, i("language_settings", n);
                    n && t && a && n.push(e),
                    i("language_settings", n)
                }
            }(t), a(null))
        })
    }
    ...
    function m() {
        var e,
            n = $('meta[name="languages"]').attr("content");
        try {
            e = JSON.parse(n)
        } catch (e) {
            Raven.captureException(e)
    ...
    function y(o) {
        ...
        return l() ? c(e) : function(n) {
            var t,
                ...
                o,
                r,
                i = $('meta[name="cmodelID"]').attr("content"),
            ...
            if (!(i || s && l))
                return Raven.captureException(new Error("Attempt to fetch examples without any valid ID available"), {
                    extra: {
                        api: t,
                        id: i,
                        ownerId: s,
    ...
    function b(e) {
        try {
            if (e.item)
                return e.item.forEach(function(e) {
                    return b(e)
                });
    ...
    function R(n) {
        ...
        if (0 < (a += "&type=meta").length && (t += a), !e)
            return Raven.captureException(new Error("Attempt to fetch Preview meta without any valid collection Id"), {
                extra: {
                    api: t,
                    uid: e
                }
    ...
    function O(n) {
        ...
        if (t && 0 != t && r.push("environment=" + t), a && r.push("versionTag=" + a), 0 < r.length && (o += "?" + r.join("&")), !e)
            return Raven.captureException(new Error("Attempt to fetch JSON without any valid ID available"), {
                extra: {
                    api: o,
                    uid: e
                }
    ...
    function A(e, t, a, o) {
        if (se.exampleResponses = {}, se.exampleRequests = {}, e) {
            ...
            if (!t && !a && !o)
                try {
                    o = getStorageItem("lang_label") || "cURL",
                    t = getStorageItem("lang_preference") || "curl",
                    a = getStorageItem("lang_variant") || "cURL"
                } catch (e) {
                    Raven.captureException(e)
                }
            try {
            ...
            if ($("#mobile-controls .language-dropdown .selected-language").text(o + " - " + a), _.isEmpty(n))
                return null === n ? "classic-double-column" === S.value ? $(E).removeClass("is-sidebar-loading").addClass("is-sidebar-error") : $(E).removeClass("are-examples-loading").addClass("is-examples-error") : "classic-double-column" === S.value ? $(E).removeClass("is-sidebar-loading") : $(E).removeClass("are-examples-loading"), $(".top-bar").removeClass("is-loading"), $(".config-bar").removeClass("is-loading"), $(".layout").removeClass("is-content-loading"), $(".config-bar").removeClass("is-content-loading"), $(".retryLoadingExamples").on("click", function() {
                    k(A)
            ...
            Array.isArray(n) && n.forEach(function(l) {
                ...
                Array.isArray(n) && n.forEach(function(e, n) {
                    ...
                    if (se.exampleRequests[l.item_id + "_" + n] = s, t) {
                        ...
                        se.exampleResponses[l.item_id + "_" + n] = t,
                        Array.isArray(a) && a.length && (se.exampleHeaders[l.item_id + "_" + n] = a.reduce(function(e, n) {
            ...
            $(".top-bar").removeClass("is-loading"),
            $(".config-bar").removeClass("is-loading"),
            $(".layout").removeClass("is-content-loading"),
            $(".config-bar").removeClass("is-content-loading"),
            "classic-double-column" === S.value ? $(E).removeClass("is-sidebar-loading is-sidebar-error") : $(E).removeClass("are-examples-loading is-sidebar-error"),
            setTimeout(function() {
                Modernizr.csspositionsticky || tn($(".pm-message-persistent")),
                Rn(),
                He(),
                Me(),
                w()
    ...
    function I(e, n) {
        var t = e,
            ...
            o = S && S.label;
        t && $(".active-environment").data("environment-id"),
        t && t.info.documentationLayout && (o = C[t.info.documentationLayout].label),
        $(".layout-dropdown .selected-layout").text(o),
        se.requestBody = {},
        ...
        pn(t, _.get(t, "info._postman_id")),
        Oe("doc-sidebar", "#doc-body" + r, t),
        Oe("nav-sidebar", "#nav-sidebar", t),
        "classic-double-column" === S.value && ($(".layout").removeClass("is-content-loading"), $(".config-bar").removeClass("is-content-loading")),
        setTimeout(function() {
            Le()
        }),
        kn(),
    ...
    function j() {
        $(".toggle-accordion-collapse").off(),
        $(".toggle-accordion-collapse").on("click", function(e) {
            var n = $(e.target).closest(".accordion"),
                t = n.hasClass("open");
            n.toggleClass("open"),
            t ? n.trigger("collapse") : n.trigger("open")
        })
    }
    function H() {
        ...
        $(".chosen-search-input").attr("maxlength", 100),
        $(".chosen-select").on("change", function(e, n) {
            !function(e, n) {
                $(".".concat(e)).trigger("change", {
                    id: e,
                    selectedValue: n
                })
            }($(this).closest(".dropdown-container").data("dropdown-id"), n.selected),
            e.stopImmediatePropagation(),
            e.preventDefault()
        })
    }
    function N(e, n) {
        $("".concat(e, " .dropdown-selector")).chosen("destroy"),
        $("".concat(e, " .dropdown-selector")).chosen(n)
    }
    function J(e, n) {
    ...
    function ee(e) {
        return "meta" === (0 < arguments.length && void 0 !== e ? e : "api") ? function() {
            ...
            Y || ne(),
            $.ajax([scope.apiHost, "api/environments", n, t].join("/")).done(function(e) {
    ...
    function te() {
        ...
        $(Q).on("change", function(e, n) {
            return function(e) {
                ...
                $(".active-environment").data("environment-id", e),
                $(".layout").removeClass("is-error"),
                Ae(),
                r ? O(function(e) {
                    In(_.get(e, "info.toc", [])),
                    I(e, t)
    ...
    function Se(r) {
        try {
            var n = ce ? O : ln,
                t = ce ? k : sn;
            if (Ae(), $(fe).addClass("is-content-loading"), $(me).removeClass("is-content-loading"), z(we, "Loading collection..."), ce)
                return R(function(e) {
                    return L(e), x(), n(function(o) {
                        In(_.get(o, "info.toc", [])),
                        z(we, "Loading snippets..."),
                        t(function(e, n, t, a) {
                            "function" == typeof r && r({
                                collectionJson: o,
                                examples: {
    ...
    function _e(e, n) {
        try {
            var t = n.data,
                ...
                o = n.langVariant,
                r = n.langLabel;
            $(fe).addClass("is-content-loading"),
            $(me).removeClass("is-content-loading"),
            z(we, "Rendering..."),
            ce ? I(e, function() {
                return re = !0
            }) : dn(e),
            re && (ce ? A(t, a, o, r) : cn(t, a, o, r), attachLanguageSelectorDropdownHandlers())
        } catch (e) {
            return Raven.captureException(e)
        }
    }
    ...
    function ke() {
        ...
        j(),
        H(),
        N(".language-selector-dropdown", {
            disable_search: !1
        }),
        _.isFunction(messenger.bootstrap) && messenger.bootstrap(messenger.showAll),
        setTimeout(function() {
            Je()
        }, 100),
        Re(),
        $(".layout-dropdown-selector").on("change", function(e, n) {
            an(n.selectedValue)
        }),
        ...
        qn(),
        _.set(Prism, "languages.pmresponseheaders", {
            kvpair: {
                pattern: /(^|\n)[^\n]*\n/,
                inside: {
                    key: /^([^:\n]+: )/
    ...
    function Re() {
        ...
        if (!e || !n || !t)
            try {
                i("lang_label", ve.label),
                i("lang_preference", ve.key),
                i("lang_variant", ve.variant)
            } catch (e) {
                Raven.captureException(e)
            }
    ...
    function He() {
        ...
        $(".example-name li").click(_n),
        $("#language-settings").on("click", function() {
            wn()
        })
    }
    function Ne(e, n) {
        if ("example-request" === n)
            return se.exampleRequests[e]
    }
    function Je() {
        var e = $("body"),
        ...
        e.on("click", ".is-expandable", mn),
        $("#mobile-controls .language_dropdown").on("click", ".dropdown-menu-item", function(e) {
            var n = $(e.target),
                t = n.data("lang-key"),
    ...
    function Pe() {
        $(".collection-description details").one("click", Ue),
        $(".folder-description details").one("click", Ue),
        $(".request-description details").one("click", Ue)
    }
    function Ue(n) {
        setTimeout(function() {
            var e = n.target;
    ...
    function Be() {
        $(".folder .toggle-folder-collapse").on("click", Sn),
        $(".folder-link>a").on("click", Xe),
    ...
    function Ve() {
        var i,
            o = $("#languageSettingsForm").data("language-key"),
        ...
        i = JSON.parse(JSON.stringify(de)),
        $("#languageSettingsForm *").filter(":input").each(function() {
            var e,
                n = $(this),
                t = n.attr("id"),
                a = n.data("type");
            if ("positiveInteger" === a) {
                ...
                "boolean" === a ? (e = n.is(":checked"), i = Fe(i, t, e)) : "enum" === a && (e = $("#" + t + " .title").text(), i = Fe(i, t, e))
        }),
    ...
    function Ge() {
        $("#languageSettingsModal .modal-header button.close").off(),
        $("#saveLanguageSettingsBtn button").off(),
        $("#languageSettingsModal .dropdown-menu-item").off(),
        $("#language-search-input").off(),
        $("#saveLanguageSettingsBtn button").on("click", function() {
            $("#saveLanguageSettingsBtn button").addClass("saving"),
            $("#saveLanguageSettingsBtn button").attr("disabled", !0),
            Ve()
        }),
    ...
    function Ke() {
        $(".languages-list-item").off(),
        $(".languages-list-item").on("click", function() {
            if (!ge) {
    ...
    function Ze() {
        $("#retryGetSettingsBtn button").off(),
        $(".ls-settings-error-container #retryGetSettingsBtn button").on("click", function() {
    ...
    function Ye(t) {
        ...
        de && de.options && r.key === de.key && r.variant === de.variant ? t(en(de.options)) : $n(r.key, r.variant, function(e, n) {
            if (e) {
                ...
                B("ERROR", "Failed to fetch settings", "Failed to fetch settings for " + (r.key + " - " + r.variant) + "."),
                t(!0)
            }
            o = (a = de = n) && a.options,
            t(en(o))
    ...
    function nn() {
        Modernizr.csspositionsticky || $(window).on("scroll resize pm-notification-closed", tn.bind(this, $(".pm-message-persistent"))),
        $(window).on("scroll", _.debounce(Rn, 1e3))
    }
    function tn(e) {
        var n = $(window).scrollTop(),
            t = e.parents(".pm-persistent-notification-container"),
            a = 0;
        e && e.length && (a = e.outerHeight()),
        70 < n ? (n = 70, t.addClass("is-fixed")) : t.removeClass("is-fixed"),
        $(".sidebar").css("padding-top", 70 + a - n + "px"),
        $(".container-fluid").css("padding-top", a + "px")
    }
    function an(e) {
        var n = e,
            t = he[n] && he[n].label,
    ...
    function on(e) {
        for (var n = window.location.search.substring(1).split("&"), t = !1, a = 0; a < n.length; a++) {
            if ("documentationLayout" === n[a].split("=")[0]) {
                n[a] = "documentationLayout=" + e.value,
                t = !0;
                break
            }
        }
    ...
    function ln(n) {
        var t,
            e = $('meta[name="cmodelID"]').attr("content"),
            a = $('meta[name="ownerId"]').attr("content"),
            o = $('meta[name="publishedId"]').attr("content"),
            r = $(".active-environment").data("environment-id"),
            i = xe(),
            s = ["segregateAuth=true"];
        if (t = !e && a && o ? scope.apiHost + "/api/collections/" + a + "/" + o : scope.apiHost + "/api/collections/" + e, r && 0 != r && s.push("environment=" + r), i && s.push("versionTag=" + i), 0 < s.length && (t += "?" + s.join("&")), !(e || a && o))
            return Raven.captureException(new Error("Attempt to fetch JSON without any valid ID available"), {
                extra: {
                    api: t,
                    id: e,
    ...
    function cn(e, t, a, n) {
        if (se.exampleResponses = {}, se.exampleRequests = {}, e) {
            ...
            if (!t && !a && !n)
                try {
                    n = v("lang_label") || "cURL",
                    t = v("lang_preference") || "curl",
                    a = v("lang_variant") || "cURL"
                } catch (e) {
                    Raven.captureException(e)
                }
            try {
            ...
            if ($("#mobile-controls .language-dropdown .selected-language").text(n + " - " + a), _.isEmpty(r))
                return null === r ? "classic-double-column" === be.value ? $($e).removeClass("is-sidebar-loading").addClass("is-sidebar-error") : $($e).removeClass("are-examples-loading").addClass("is-examples-error") : "classic-double-column" === be.value ? $($e).removeClass("is-sidebar-loading") : $($e).removeClass("are-examples-loading"), $(pe).removeClass("is-loading"), $(me).removeClass("is-loading"), $(fe).removeClass("is-content-loading"), $(me).removeClass("is-content-loading"), $(".retryLoadingExamples").on("click", function() {
                    ce ? k(A) : sn(cn)
                }), void $("#language-settings").on("click", function() {
                    wn()
                });
            ...
            !0,
            "classic-double-column" === be.value ? $($e).removeClass("is-sidebar-loading is-sidebar-error") : $($e).removeClass("are-examples-loading is-examples-error"),
            $(pe).removeClass("is-loading"),
            $(me).removeClass("is-loading"),
            $(fe).removeClass("is-content-loading"),
            $(me).removeClass("is-content-loading"),
            setTimeout(function() {
                Modernizr.csspositionsticky || tn($(".pm-message-persistent")),
                Rn(),
    ...
    function dn(e, n) {
        var t = e,
            a = e && e.item,
            o = be && be.label;
        t && t.info.public && (0 == $(".active-environment").data("environment-id") ? "no-env" : "env", t.info.privateUrl),
        $(".layout-dropdown .selected-layout").text(o),
        se.requestBody = {},
        a.forEach(function(e) {
            gn(e)
        ...
        Oe("nav-sidebar", "#nav-sidebar", t),
        re = !0,
        "classic-double-column" === be.value && ($(fe).removeClass("is-content-loading"), $(me).removeClass("is-content-loading")),
        setTimeout(function() {
            Le()
        }),
        kn(),
    ...
    function gn(e) {
        ...
        pn(e, t),
        n && (se.requestBody[t] = n),
        e && e.item && e.item.forEach(function(e) {
            gn(e)
        })
    }
    function pn(e, n) {
        var t,
            a = !!e.item;
        t = fn(a ? _.get(e, "auth") : _.get(e, "request.auth"), n, a ? "folder" : "request"),
        e.authInfoForDisplay = t
    }
    function fn(e, n, t) {
    ...
    function mn(e) {
        ...
        if (e && e.target && "A" === e.target.tagName) {
            if (e.ctrlKey || e.metaKey) {
                var b = window.open();
                b.opener = null,
                b.location = e.target.href
            } else
                window.open(e.target.href, "_self");
            return !1
        }
        if (r = 0 !== $(this).parents(".examples").length ? (l = "dark-background", "") : (l = "white-background", "section"), n = g.css("width"), t = g.data("id"), d = g.data("content-type"), a = g.data("title"), o = g.data("clipboard-target"), i = 0 <= ["html", "xml"].indexOf((g.data("lang") || "").toLowerCase()), s = g.clone(), c = se.exampleResponses[t], u = se.exampleHeaders[t], h.attr("data-clipboard-target", ""), this.classList.contains("is-example"))
            v.empty().text(c),
            p.removeClass("white-background dark-background").addClass(l).modal().find(".modal-body").css("width", n || "auto"),
            $("#snippetModal .modal-header .title").empty().text(a),
            c.replace(/\s/gi, "").length <= le.maxHighlightLength && (v.removeClass("language-markup language-javascript language-undefined language-pmresponseheaders").addClass("language-" + (i ? "markup" : "javascript")), Prism.highlightElement(v.get(0)));
        else if (this.classList.contains("is-example-headers"))
            v.empty().text(u),
            p.removeClass("white-background dark-background").addClass(l).modal().find(".modal-body").css("width", n || "auto"),
            $("#snippetModal .modal-header .title").empty().text(a),
            u.replace(/\s/gi, "").length <= le.maxHighlightLength && (v.removeClass("language-markup language-javascript language-undefined language-pmresponseheaders").addClass("language-pmresponseheaders"), Prism.highlightElement(v.get(0)));
        else {
            var w = "",
                x = "";
            if (x = (w = "example-request" === d ? se.exampleRequests[t] : se.requestBody[t]) && w.mode && "graphql" !== w.mode ? w[w.mode] : w && "graphql" === w.mode ? "graphql-variables" === d ? w.graphql.variables : w.graphql.query : w, n = s.css("width"), s.removeClass("is-expandable click-to-expand-wrapper is-snippet-wrapper"), !x)
                return m.addClass("white-background").modal().find(".modal-body").empty().append(s).css("width", n || "auto"), !1;
            o ? (h.show(), h.attr("data-clipboard-target", "#modal-" + g.data("id"))) : h.hide(),
            y.empty().text(x),
            y.attr("id", ""),
            o && y.attr("id", "modal-" + g.data("id")),
            f.removeClass("white-background dark-background").addClass(l).modal().find(".modal-body").removeClass("section").addClass(r).css("width", n || "auto"),
            $("#rawBodyModal .modal-header .title").empty().text(a),
            x && x.replace(/\s/gi, "").length <= le.maxHighlightLength && (y.removeClass("language-markup language-javascript language-undefined").addClass("language-" + (i ? "markup" : "javascript")), Prism.highlightElement(y.get(0)))
        }
    }
    function vn(e, n, t) {
        var a,
            ...
            i,
            s,
            l = $("#languageSettingsModal .languages-list"),
            c = m();
        if (e && n && t)
            o = e,
            a = n,
            ...
            try {
                o = v("lang_label") || "cURL",
                a = v("lang_preference") || "curl",
                r = v("lang_variant") || "cURL"
            } catch (e) {
                Raven.captureException(e)
            }
        c = c || [],
        i = Handlebars.partials["_language-list"]({
            languages: c,
            language: {
                key: a,
                label: o,
    ...
    function hn(e, n, t, a, o) {
        var r,
            i,
            s,
            l = $("#languageSettingsModal .language-settings"),
            c = $("#saveLanguageSettingsBtn button"),
            u = {};
        if (ge = !0, "saveSettings" !== a && (l.addClass("is-loading"), c.removeClass("saving"), c.attr("disabled", !1)), e && n && t)
            i = e,
            r = n,
            s = t;
        else
            try {
                i = v("lang_label") || "cURL",
                r = v("lang_preference") || "curl",
                s = v("lang_variant") || "cURL"
            } catch (e) {
                Raven.captureException(e)
            }
        return $n(r, s, function(e, n) {
            if (e ? bn(r, i, s) : (de = u = n, yn(u)), "saveSettings" === a)
                return o && o()
        })
    }
    function yn(e) {
        ...
        ge = !1,
        n = Handlebars.partials["_language-settings"]({
            language: e
        }),
        t.replaceWith(n),
        setTimeout(function() {
    ...
    function bn(e, n, t, a) {
        var o,
            r = $("#languageSettingsModal .language-settings"),
            i = $("#saveLanguageSettingsBtn button");
        r.removeClass("is-loading"),
        ge = !1,
        o = Handlebars.partials["_language-load-error"]({
            language: {
                key: e,
                label: n,
                variant: t
    ...
    function wn() {
        ...
        $("#languageSettingsModal").on("hidden.bs.modal", Qe),
        vn(),
        hn()
    }
    function xn(e) {
    ...
    function _n() {
        ...
        $(".formatted-requests[data-id^=" + t + "]").hide(),
        $(".example-response[data-id^=" + t + "]").hide(),
        $(".formatted-requests[data-id=" + n + "]").show(),
        $(".example-response[data-id=" + n + "]").show(),
        $("#" + t + "_dropdown").attr("title", e.text()),
        $("#" + t + "_dropdown .example-name__label").text(e.text()),
        Rn(),
        Me()
    }
    function En(e, n, t) {
    ...
    function Rn() {
        Ye(function(e) {
            ...
            _.forEach(a, function(e) {
                var n;
                -1 === _.indexOf(t, e) && (n = $(e)).text(n.text()).removeClass("is-highlighted")
            }),
    ...
    function qn() {
        $(".accordion").on("collapse", function(e) {
            var n = $(e.target).find(".sidebar-icons"),
                t = n.children()[0],
                a = n.children()[1];
            $(t).removeClass("pm-icon-triangle--open").addClass("pm-icon-triangle--closed"),
            $(a).removeClass("pm-icon-folder--open").addClass("pm-icon-folder--closed")
        }),
        $(".accordion").on("open", function(e) {
            var n = $(e.target).find(".sidebar-icons"),
                t = n.children()[0],
                a = n.children()[1];
            $(t).removeClass("pm-icon-triangle--").addClass("pm-icon-triangle--open"),
            $(a).removeClass("pm-icon-folder--").addClass("pm-icon-folder--open")
        })
    }
    $.ajaxSetup({
        dataType: "json",
    ...
    Boolean(window.scope.userId) && $.ajax("".concat(scope.identityPublicApi, "/api/check-backfill-status"), {
        ...
        if (401 === e.status)
            return $.ajax("".concat(scope.identityPublicApi, "/api/test-third-party-cookie"), {
                dataType: "text"
            }).then(function() {
    ...
    $(document).ready(function() {
        return ce = l(), ue = function() {
            ...
            _e(e.collectionJson, e.examples),
            ke()
        })
    })
}]);
